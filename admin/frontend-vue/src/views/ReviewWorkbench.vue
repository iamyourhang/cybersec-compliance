<template>
  <div class="review-page">
    <section class="card review-hero">
      <div>
        <div class="card-title">审核案例</div>
        <div class="review-title">真实性审核工作台</div>
        <div class="review-subtitle">
          这里围绕审核案例、证据链和处理结论工作。候选、待复核、已隔离状态都在这里收口，不再混在知识库编辑里。
        </div>
      </div>
    ...
    <section class="review-grid">
      <div class="card cases-card">
        <div class="panel-head">
          ...
          <div class="panel-caption">优先看真实性风险高和证据不足的项</div>
        </div>
        ...
        <div v-else class="case-list">
          ...
          >
            <div class="case-row-head">
              ...
              <span class="case-status" :class="item.current_status" :title="item.current_status">{{ authenticityLabel(item.current_status) }}</span>
            </div>
            <div class="case-row-meta">
              <span>{{ item.country_code || '—' }}</span>
              <span>{{ item.entry_type || '—' }}</span>
              <span>证据风险 {{ item.risk_score ?? 0 }}</span>
            </div>
      ...
      <div class="card detail-card">
        ...
        <template v-else>
          <div class="detail-header">
            ...
            <span class="case-status large" :class="selectedCase.current_status" :title="selectedCase.current_status">{{ authenticityLabel(selectedCase.current_status) }}</span>
          </div>

          <div class="summary-grid">
            <div class="summary-card">
              <div class="summary-label">证据风险分</div>
            ...
            <div class="summary-card">
              ...
              <div class="summary-value small" :title="selectedCase.source_download_status">{{ downloadStatusLabel(selectedCase.source_download_status) }}</div>
            </div>
          </div>
          ...
          <div class="detail-section">
            ...
            <div v-else class="event-list">
              <div v-for="event in evidence.events.slice(0, 8)" :key="event.id" class="event-card">
                ...
                <div class="event-flow">{{ event.from_status ? authenticityLabel(event.from_status) : '—' }} → {{ event.to_status ? authenticityLabel(event.to_status) : '—' }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="action-bar">
            ...
            <button class="btn btn-outline" @click="prefillDecision('suspicious')">标为待复核</button>
          ...
          <div class="detail-section" v-if="aiAssist">
            ...
            <div class="artifact-card">
              <div class="artifact-meta">
                <span :title="aiAssist.evidence_status || 'analysis'">{{ statusLabel(aiAssist.evidence_status || 'analysis') }}</span>
        ...
        <div class="modal-body" style="display:grid;gap:12px">
          ...
          <div class="form-group">
            ...
            <div style="margin-top:6px;font-size:12px;color:var(--text3)">
              {{ manualSourceFile ? `已选择：${manualSourceFile.name}` : '当服务器抓不到时，可直接从本机上传官方 PDF 或正文页 HTML。' }}
            </div>
          </div>
          <div class="form-group">
            <label class="form-label">证据备注 *</label>
...
import { artifactTypeLabel, authenticityLabel, downloadStatusLabel, statusLabel } from '@/utils/labels'
...
async function submitManualSource() {
  ...
  try {
    if (manualSourceFile.value) {
      ...
      formData.append('official_url', manualSourceForm.official_url.trim())
      formData.append('evidence_note', manualSourceForm.evidence_note.trim())
      formData.append('auto_parse', 'true')
      formData.append('file', manualSourceFile.value)
      await complianceApi.manualSourceUpload(selectedCase.value.compliance_id, formData)
    } else {
